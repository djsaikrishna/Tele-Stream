...
def _year_label(item: dict) -> str:
    ...
    if isinstance(start, str) and "-" in start.strip():
        ...
        try:
            ...
            if not end and len(parts) > 1 and parts[1].strip().isdigit():
                end = parts[1].strip()
        except (TypeError, ValueError):
            return start.strip()
    else:
...
async def get_meta(token: str, media_type: str, id: str, token_data: dict = Depends(verify_token)):
    ...
    try:
        ...
        LOGGER.error(f"[META] lookup failed for {imdb_id}: {e}")
        return {"meta": {}}
    ...
    meta_obj = {
        ...
        "imdb_id": media.get("imdb_id") or imdb_id,
